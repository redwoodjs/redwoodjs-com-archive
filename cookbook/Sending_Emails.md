...
When that's done, go into the `email` directory and we'll install the `nodemailer` package.
...
Now, fire up your editor of choice and find the `schema.prisma` file and remove the example model. The app we're building is going to have two models. One for our users and one for the audit logs.
...
One of Redwood's stand-out features is the scaffolds. We'll be using that here to quickly get a nice visual list of the users in our database to work with.
...
Go ahead and create a few users. Since we're going to send emails to these users, use emails you can actually check. So you can make sure it works. A service I like to use for generating random users with real emails is https://www.fakenamegenerator.com. Just click the link on that page to activate the email address and you'll be able to send emails from your app, and see them arrive.
...
To add our button, and the actions connected to it, we need to add a fair bit of code to the User component. I'll put the full code to make sure you don't miss anything.
...
And then in the users service we'll just create a dummy method for now
...
Now let's go back to the service and add the missing pieces there. At the top, after the db import, add the `sendEmail` import
...
You can now test your app's new email sending capabilities by clicking on the email button you added previously. You should see a "Sending email to: horacebcarrier@teleworm.us" message in your console, and a few minutes later it should pop up in the users email inbox. (If you're using the emails generated by fakenamegenerator you need to be patient, it does take a while before you can see new emails arriving.)
