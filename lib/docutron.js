...
const splitToPages = (markdown, book) => {
  const sections = [];
  let buffer = [];
...
const create = (markdown, book) => {
  console.log(`Building ${book} docs...`);
  console.group();

  const markdownPages = splitToPages(markdown, book);
  const htmlPages = convertToHtml(markdownPages);
  buildNav(markdownPages, book);

  htmlPages.forEach(page => {
    fs.writeFileSync(path.join(HTML_ROOT, page.href), page.html);
    console.log(`+ Wrote ${book}:${page.title}`);
  });
