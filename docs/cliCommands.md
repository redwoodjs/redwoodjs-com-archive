...
Execute scripts in the CLI to run one-off operations, long running jobs and utility scripts. 
...
### Background worker with faktory

Using the redwood exec feature, we will create a background worker using an external library called Faktory. At a high level, Faktory is Language-agnostic persistent background job server. Learn more at https://github.com/contribsys/faktory. The server can be run using Docker - https://github.com/contribsys/faktory/wiki/Docker.


For our client, we will use the node library https://github.com/jbielick/faktory_worker_node. We will use this library to send jobs from our redwood application to our Faktory server. This libary can also be configured with redwood exec feature to process the jobs from the Faktory server.


Let's create our faktory worker to consume the tasks, share our redwood application code and perform the work. We are going to register a task called `postSignupTask` in our worker.
...
Now let's create the task in the `lib/tasks` folder. In the `postSignupTask` we may want to perform operations that contact external services such as sending an email. This type of work, we typically don't want to hold up the request/response cycle and can perform in the background. 
...
Once we have created our task, we need to call it in the right place. For this task, it makes sense to call it right after the user has completed their signup. This is an example of a service that will be most likely called through a GraphQL Mutation. 
...
export const signUp = async ({ input }) => {
  // ... perform all the signup operations such as creating entry in DB and auth provider.

  // send our task to the Faktory server
...
That's it - we are done.  Run your Faktory server using Docker, and run the worker using `yarn rw exec faktoryWorker`

If your Faktory server in running and you have set the `FAKTORY_URL` correctly. You will see the server pick up the jobs and your worker process the job.
