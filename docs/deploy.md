...
Redwood is designed for serverless architecture deployment, which requires a unique continuous deployment process:
...
To deploy Redwood on Netlify, see the [Tutorial Deployment section](https://deploy-preview-295--redwoodjs.netlify.app/tutorial/deployment).
...
3. follow the applicable steps in the deployment section of the Tutorial (see link above)

## Vercel
_The following instructions assume you have read the [General Deployment Setup](#general-deployment-setup) section above._

### Project Init
If you haven't already, we recommend experiencing the full Redwood DX via the [Redwood Tutorial](https://redwoodjs.com/tutorial/welcome-to-redwood). Simply return to these instructions when you come to the Deployment section.
...
Complete the two steps below. Then save, commit, and push changes to your repo.

**1. Serverless Functions Path**  
In your `redwood.toml` file, set your `apiProxyPath`:
```js
apiProxyPath = "/api"
```

**2. Database Settings**  
Follow the steps in the [Prisma and Database](#prisma-and-database) section above.

_Skip these settings if you do not have or require a database._
